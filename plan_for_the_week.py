...
def generate():
    maximum = 5
    distribute_table = [i for multiplayer in range(1, maximum)
...
for number_of_elements, number_of_day in zip(generate(), range(7)):
    if not pass_in_next_day:
        api.add_task(content=f'Poświęć mroku {number_of_elements}', project_id=project_id, due_string=f'+{number_of_day + 1} day')
